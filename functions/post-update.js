...
module.exports.handler = async (event) => {
  ...
  try {
    const FLOW_URL   = process.env.FLOW_POST_UPDATE_URL;
    const CLIENT_KEY = process.env.CLIENT_KEY;

    if (!FLOW_URL || !CLIENT_KEY) {
      return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'CONFIG', message: 'Falta FLOW_POST_UPDATE_URL o CLIENT_KEY' }) };
    }

    const input = event.body ? JSON.parse(event.body) : {};

    // Enviar la key en el BODY (como usa Vercel). Incluimos ambos nombres por compatibilidad.
    const payload = {
      ...input,
